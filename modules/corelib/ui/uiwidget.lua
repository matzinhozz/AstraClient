...
function UIWidget:__applyOrBindHtmlAttribute(attr, value, isInheritable, controllerName, NODE_STR)
    ...
    if attr == '*if' or attr == '*condition-if' then
        local success = true
        ...
        local function applyConditionIf(widget, result)
            result = result and true or false
            if widget.setResultConditionIf then
                widget:setResultConditionIf(result)
            elseif widget.setDisplay then
                widget:setDisplay(result and 'block' or 'none')
            else
                widget:setVisible(result)
            end
        end

        applyConditionIf(self, value)
        WidgetWatch.register({
            widget = self,
            res = value,
            method = applyConditionIf,
            methodName = 'conditionif',
            attr = 'if',
            htmlId = self:getHtmlId(),
            valueExpr = valueExpr,
            fnc = function(self)
                local result = fnc(controller, self.widget, unpack(self.valueExpr))
                if result ~= self.res then
                    self.method(self.widget, result)
                    self.res = result
                end
            end
        })
