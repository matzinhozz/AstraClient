...
function ImbuementItem.setup(itemId, tier, slots, activeSlots, availableImbuements, needItems, itemName)
    self.itemId = itemId
...
function ImbuementItem.configureWindow(window)
        ...
        if slotWidget then
            ...
            if i <= self.slots then
                ...
                if imbuement and imbuement[1] and imbuement[1].id ~= 0 then
                    slotWidget.resource:setImageClip(getFramePosition(imbuement[1]["imageId"], 64, 64, 21) .. " 64 64")
                end
            else
    ...
    if not itemName or itemName == "" or itemName == "item" then
        itemName = getItemNameById(self.itemId)
    end
...
function ImbuementItem.onSelectSlotClear(imbuement)
    ...
    widget.resource:setImageClip(getFramePosition(imbuement[1]["imageId"], 64, 64, 21) .. " 64 64")
